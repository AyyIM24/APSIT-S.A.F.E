...
const AdminFoundItems = ({ items, addItem }) => {
  const [showModal, setShowModal] = useState(false);
  const [qrItem, setQrItem] = useState(null);
  const [formData, setFormData] = useState({
    name: '', category: '', location: '', date: '', description: '',
    type: 'found', status: 'SECURED', reporter: 'Admin',
  ...
  return (
    <>
      <div className="admin-page-header">
        <h2 className="admin-page-title">Found Items ({foundItems.length})</h2>
        ...
        {foundItems.map((item, index) => (
          ...
          >
            <div className="card-image-field">
              ...
              <span className="status-badge" style={{ background: '#28a745' }}>{item.status}</span>
            </div>
            <div className="card-info" style={{ background: 'rgba(102,126,234,0.15)' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
                <Link to={`/item/${item.id}`} style={{ flex: 1 }}>
                  <button className="btn-gradient-card" style={{ width: '100%' }}>View</button>
                </Link>
                <button
                  className="btn-gradient-card"
                  style={{ flex: 1, background: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' }}
      ...
      {/* QR Modal */}
      {qrItem && (
        <AdminQRView item={qrItem} onClose={() => setQrItem(null)} />
      )}
          ...
          <div className="admin-modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="admin-modal-form" onSubmit={handleSubmit}>
              <div>
                ...
                <input type="text" name="name" required onChange={handleChange} placeholder="e.g. iPhone 13" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" name="location" required onChange={handleChange} placeholder="e.g. Canteen" />
              </div>
              <div>
